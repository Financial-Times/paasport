...
./deploy.py $ID
curl -XPOST http://ec2-52-17-74-125.eu-west-1.compute.amazonaws.com:8080/paasport/code-deploy/$ID/deployments -H 'Accept: application/json' -H 'Content-Type: application/json' -d'{"sourceTar":{"url":"https://s3-eu-west-1.amazonaws.com/paasport-code-deploy/demo-app-java.tar","version":"not-used-just-yet" }}'
